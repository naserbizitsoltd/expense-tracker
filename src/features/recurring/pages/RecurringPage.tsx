  ...
  return (
    <div className="flex min-h-full flex-col bg-[#0b0f18] px-4 pb-24 pt-6">
      <div className="mb-4 flex items-center gap-3 px-1">
        ...
        <h1 className="flex-1 text-xl font-semibold text-white">Recurring</h1>
        <button
          onClick={() => {
            setEditingRule(null)
            setFormOpen(true)
        ...
        {(['expense', 'income'] as const).map((t) => (
          <button
            ...
            className={`flex-1 rounded-lg py-2 text-sm font-semibold capitalize transition-all duration-150 ${
              tab === t ? 'bg-emerald-400 text-black' : 'text-white/50'
            }`}
          >
            {t}
          </button>
      ...
      {!isLoading && items.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-white/5 bg-white/[0.02] px-6 py-12 text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-white/5">
            <Repeat size={22} className="text-white/40" />
          </span>
          <p className="text-sm font-medium text-white">No recurring {tab === 'expense' ? 'expenses' : 'income'} yet</p>
