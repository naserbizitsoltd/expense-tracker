...
interface DpsPayoutSheetProps {
  ...
  onClose: () => void
  dps: Dps
  deposited: number // total real deposited amount (see dpsService.getDpsDeposited) — never recalculated here
  onReceived: (payout: DpsPayout, account: Account) => void
}

// Wrapped in singleFlight once, at module scope — same pattern as
// DpsContributionSheet's submitContribution.
      ...
      <BottomSheet open={open} onClose={resetAndClose} title="Receive Maturity">
        <div className="flex max-h-[70vh] flex-col gap-5 overflow-y-auto pb-1 pr-0.5">
          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5">
            ...
            <p className="mt-1 text-xs text-white/40">Deposited</p>
            <p className="text-lg font-semibold tabular-nums text-white">{formatAmount(deposited, dps.currency)}</p>
          </div>

          <div>
            <label className="mb-1.5 block px-1 text-xs font-medium text-white/50">Profit / Interest (optional)</label>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5">
            ...
            <p className="text-lg font-semibold tabular-nums text-white">{formatAmount(maturityAmount, dps.currency)}</p>
          </div>
