...
export function BudgetDetailsSheet({ budgetId, onClose, onEdit }: BudgetDetailsSheetProps) {
  ...
  return (
    ...
    >
      ...
      {!isLoading && !details && <p className="py-8 text-center text-sm text-white/40">Budget not found.</p>}
        ...
        <div className="flex flex-col gap-5 pb-2">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-white">{details.categoryName}</p>
          ...
          <div className="rounded-2xl border border-white/5 bg-white/[0.03] px-4 py-4">
            <div className="mb-2 flex items-end justify-between">
              <p className="text-2xl font-semibold text-white">{formatAmount(details.spent, details.budget.currency)}</p>
              <p className="text-xs text-white/40">of {formatAmount(details.budget.amount, details.budget.currency)}</p>
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              <span className={details.remaining < 0 ? 'text-rose-300' : 'text-white/50'}>
                {details.remaining < 0
              ...
              <span className="text-white/40">{details.percentUsed}% used</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl border border-white/5 bg-white/[0.02] px-3 py-2.5">
              <p className="text-[10px] uppercase tracking-wide text-white/30">Start date</p>
              <p className="text-white">{format(details.budget.startDate, 'd MMM yyyy')}</p>
            </div>
            <div className="rounded-xl border border-white/5 bg-white/[0.02] px-3 py-2.5">
              <p className="text-[10px] uppercase tracking-wide text-white/30">End date</p>
              <p className="text-white">{details.budget.endDate ? format(details.budget.endDate, 'd MMM yyyy') : 'No end date'}</p>
            </div>
          </div>
          ...
          {details.budget.notes && (
            <div className="rounded-xl border border-white/5 bg-white/[0.02] px-3 py-2.5">
              ...
              <p className="text-sm text-white/70">{details.budget.notes}</p>
            </div>
          )}
          ...
          <div>
            ...
            {details.relatedExpenses.length === 0 ? (
              <p className="px-1 py-4 text-center text-xs text-white/30">No expenses in this period yet.</p>
            ) : (
              <div className="flex flex-col gap-1">
                {details.relatedExpenses.map((t) => (
                  <div key={t.id} className="flex items-center justify-between rounded-xl px-2 py-2 text-sm">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-white/80">{t.note || details.categoryName}</p>
                      <p className="text-xs text-white/30">{format(t.date, 'd MMM, h:mm a')}</p>
                    </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 py-3 text-sm font-semibold text-white"
          >
