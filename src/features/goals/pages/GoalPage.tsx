  ...
  return (
    <div className="flex min-h-full flex-col bg-[#0b0f18] px-4 pb-24 pt-6">
      <div className="mb-4 flex items-center gap-3 px-1">
        <button onClick={onBack} aria-label="Back" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/5 text-white/70 active:bg-white/10">
          <ArrowLeft size={18} />
        </button>
        <h1 className="flex-1 text-xl font-semibold text-white">Goals</h1>
        <button
          onClick={() => setShowArchived((v) => !v)}
          aria-label="Show archived goals"
          className={cn('flex h-9 w-9 items-center justify-center rounded-full', showArchived ? 'bg-white/15 text-white' : 'bg-white/5 text-white/50')}
        >
          <Archive size={16} />
        </button>
        <button
      ...
      {!isLoading && totalGoals > 0 && !showArchived && (
        <div className="mb-5 rounded-2xl border border-white/5 bg-white/[0.03] px-4 py-4">
          <div className="mb-3 flex items-end justify-between">
            <div>
              <p className="text-2xl font-semibold text-white">{formatAmount(totalSaved, 'BDT')}</p>
              <p className="text-xs text-white/40">of {formatAmount(totalTarget, 'BDT')} targeted</p>
            </div>
            <p className="text-sm font-medium text-emerald-300">{overallProgress}%</p>
          </div>
          <div className="flex items-center gap-4 border-t border-white/5 pt-3 text-xs">
            <span className="text-white/50">
              <span className="font-semibold text-white">{totalGoals}</span> goal{totalGoals === 1 ? '' : 's'}
            </span>
            <span className="text-white/50">
      ...
      {!isLoading && items.length === 0 && !showArchived && (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-white/5 bg-white/[0.02] px-6 py-12 text-center">
          ...
          <p className="text-sm font-medium text-white">No savings goals yet</p>
          <p className="text-xs text-white/40">Create a goal and start building towards it.</p>
          <button
            onClick={() => {
              setEditingGoal(null)
              setFormOpen(true)
      ...
      {!isLoading && items.length === 0 && showArchived && (
        <p className="py-8 text-center text-sm text-white/40">No archived goals.</p>
      )}
